...
public class ModificarPrestamo extends javax.swing.JFrame {
     ...
     */
    public ModificarPrestamo(String[] prestamo, StmtSQL cxn, Prestamos vPrestamos) {
        ...
        txtf_tituloLibro.setText(prestamo[3]);
        txtf_nombreMiembro.setText(prestamo[4]);
        try{
            java.util.Date fechaPrestamo = new java.text.SimpleDateFormat("yyyy-MM-dd").parse(prestamo[5]);
            java.util.Date fechaEntrega = new java.text.SimpleDateFormat("yyyy-MM-dd").parse(prestamo[6]);
            dc_fechaPrestamoLibro.setDate(fechaPrestamo);
            dc_fechaEntregaLibro.setDate(fechaEntrega);
    ...
    private void initComponents() {
        ...
        lbl_titleRegistroPrestamo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/icons/icons8_edit_property_25px_5.png"))); // NOI18N
        lbl_titleRegistroPrestamo.setText("Modicar Préstamo");
    ...
    private void obtenerDatosPrestamo(){
        nPrestamo[0] = prestamo[0];
        if(sLibro==null)
            nPrestamo[1] = prestamo[1];
        else
            nPrestamo[1] = sLibro;
        if(sMiembro==null)
            nPrestamo[2] = prestamo[2];
        else
            nPrestamo[2] = sMiembro;
        Calendar f = dc_fechaPrestamoLibro.getCalendar();
        nPrestamo[3] = f.get(Calendar.YEAR)+"-"+(f.get(Calendar.MONTH)+1)+"-"+f.get(Calendar.DATE);
        Calendar f1 = dc_fechaEntregaLibro.getCalendar();
        nPrestamo[4] = f1.get(Calendar.YEAR)+"-"+(f1.get(Calendar.MONTH)+1)+"-"+f1.get(Calendar.DATE);
    }
    
    private void btn_guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_guardarActionPerformed
        obtenerDatosPrestamo();
        try {
            cxn.modificarPrestamo(nPrestamo);
            JOptionPane.showMessageDialog(null, "Se ha modificado el registro del préstamo.", "Codex",1);
            vPrestamos.setTableData("");
        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "ERROR: "+ex.getMessage(), "Codex",1);
        }
        btn_cancelarMouseClicked(null);
